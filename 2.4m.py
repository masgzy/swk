...
mir = 'http://allmc.eu.org/'
iurl = mir + "https://img.siwake.cc/"
wurl = mir + "http://siwake.cc/"
...
def save_to_file(link, title):
    ...
    # 替换链接中的原始网址为镜像网址
    link = link.replace('https://img.siwake.cc/', iurl)

    # 确保标题中没有无法编码的字符
    try:
        title.encode('GBK')
    except UnicodeEncodeError:
        title = title.encode('GBK', 'replace').decode()  # 将无法编码的字符替换为问号

    encoding = 'GBK' if sys.platform.startswith('win') else 'utf-8'
    
    new_command = f"wget -b -q -O \"{title}.zip\" \"{link}\"\n"
...
def scrape_site(base_urls):
    ...
    with Progress(
        ...
        task = progress.add_task("[cyan]抓取中...", total=710) #859
...
if __name__ == "__main__":
    ...
    a = input("1.白丝 2.黑丝 3.肉丝 4.灰丝 5.咖丝 6.网袜 7.自选机构 8.搜索 9.所有 10.自定义 0.退出\n选择一个选项: ")
    base_urls = []
    ...
    if a in categories:
        if a == '7':
            clear_screen()
            print("选择机构（输入对应的数字）:")
            for num, name in categories['7'].items():
                print(f"{num}. {name}")
            b = input("输入机构编号: ")
            if b in categories['7']:
                base_urls = [ wurl + 'category-' + categories['7'][b]]
            else:
                console.log("无效的机构编号。")
                sys.exit()
        elif a == '8':
            ...
            base_urls = [wurl + 'tags-' + categories[a]]
